...
// data
// {'sortedScoreBoard': [{'playerName': 'playerName1', 'playerScore': 15},
// { 'playerName': 'playerName2', 'playerScore': 19 }]}

export default function Score({
  lang, isScoreOpen, setScoreOpen, gameWinner, setGameWinner,
}: ScoreProps): any {
  const [sortedScoreBoard, setSortedScoreBoard] = useLocalStorage(
    'REACT_GAME_SORTED_SCOREBOARD',
    '{"sortedScoreBoard": []}',
  );
  ...
  function increasePlayerScore(playerName: string) {
    ...
    if (sortedScoreBoardArray) {
      const playerScoreIndex: any = sortedScoreBoardArray.findIndex(
        (x: { playerName: string }) => x.playerName === playerName,
      );
      if (playerScoreIndex !== -1) {
        sortedScoreBoardArray[playerScoreIndex] = {
          playerName,
          playerScore: sortedScoreBoardArray[playerScoreIndex].playerScore + 1,
        };
        sortedScoreBoardArray.sort((a: { playerScore: number },
          b: { playerScore: number }) => (b.playerScore > a.playerScore ? 1 : -1));
      } else {
      ...
      {isScoreOpen && (
        <div className='modal score'>
          <div className='modal__body'>
            ...
            <ol className='score__table'>{getReactScores(sortedScoreBoard)}</ol>
